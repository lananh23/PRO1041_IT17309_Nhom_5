...
import Services.UltiClass;
import Utilities.hoaDonPDF;
import static Utilities.hoaDonPDF.xuatHoaDon;
...
public class FrmBanHang extends javax.swing.JFrame {
    ...
    private ManageHoaDonService hdSV;
    ...
    public FrmBanHang() {
        ...
        khService = new KhachHangSV();
        hdSV = new ManageHoaDonService();
...
//        }
    ...
    private void btnTimSPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTimSPActionPerformed
        ...
        if (maSP == null || maSP.isEmpty()) {
            ...
            List<QLSanPham> sanPhams = banHangService.findByCode(maSP);
    ...
    private void btnThanhToanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThanhToanActionPerformed
        ...
        if(maKH.equals(khService.select1(maKH))){
            String tenKH = khService.select1(maKH).toString();
        }
   
        List<ManageHoaDonChiTiet> list = new ArrayList<>();
        int row = tblDSSanPham.getSelectedRow();
        ManageHoaDonChiTiet hdct = new ManageHoaDonChiTiet();
        String ma = txtMaKH.getText();
        List<KhachHang> kh = khService.select1(ma);
          
        KhachHang k = kh.get(0);
        String tenKH = k.getTenKH();
        kh.add(k);
        JOptionPane.showMessageDialog(this, tenKH);
      
        hdct.setMaHDCT(txtMaHDCT.getText());
        hdct.setMaSP(lbMaSP.getText());
        hdct.setSoLuong((int) spnSoLuong.getValue());
        hdct.setGiaBan((Float) tblDSSanPham.getValueAt(row, 4));
        HoaDon hDon = new HoaDon();
        float tongTien = hd.getTongTien();
        list.add(hdct);
        try {
            xuatHoaDon(new HoaDon(txtMaHD.getText()),new KhachHang(txtMaKH.getText(),tenKH ), list,UltiClass.getTongTienHoaDon(list));
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Lỗi");
        }
...
//        }
    ...
    private void btnTimActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTimActionPerformed
        // TODO add your handling code here:
        String sdt = JOptionPane.showInputDialog("Nhập số điện thoại của khách hàng", null);
        List<KhachHang> kh = khService.searchMaKH(sdt);
        txtMaKH.setText(sdt);
    ...
    private void btnThemKHActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemKHActionPerformed
        // TODO add your handling code here:
        FrmThemNhanhKhachHang kh = new FrmThemNhanhKhachHang();
        kh.setVisible(true);
